...
The one exception is a TLDR. End every response with a line starting `**TLDR:**` that says the outcome in one or two plain sentences. It's the only summary allowed, so keep it short and don't add another closer before it.
